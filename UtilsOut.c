...
PUBLIC void vUtils_TrueDisplayMsg(uint32 u32Data)
{
...
PUBLIC void vUtils_DisplayHexNoZero(uint32 u32Data, int iSize)
{
    char acValue[9];
...
PUBLIC void vUtils_ValToHexNoZero(char *pcString, uint32 u32Data, int iSize)
{
    uint8 u8Nybble;
    int i, j;
    j = 0;
...
PUBLIC void vUtils_ValToDec(char *pcOutString, uint8 u8Value)
{
    ...
    uint8 u8DigitIndex;
    uint8 u8Count;
    bool_t boPreviousDigitPrinted = TRUE;// FALSE;
